...
class StaticCodeChecker:
    def is_static(self, cell, namespace):
        raise NotImplementedError("Abstract method")
    
class AlwaysNonStaticCodeChecker:
    def is_static(self, cell, namespace):
        return False

class AllowlistStaticCodeChecker:
    def is_static(self, cell, namespace):
        print(cell)
        code = ast.parse(cell)
        static_checker = StaticChecker(RULES, namespace)
        return static_checker.visit(code, None)
...
class StaticChecker(ast.NodeVisitor):
    ...
    def is_static(self, node, parent):
        if isinstance(node, StaticChecker.DISALLOW):
            return False
        if isinstance(node, StaticChecker.ALLOW):
            ...
            return True
        if isinstance(node, StaticChecker.MATCH):
    ...
    def _match(self, node: ast.AST, parent: ast.AST):
        for rule in self.rules:
            ...
            if node_type == ast.Attribute and isinstance(node, node_type) and node.attr == value and self.query_namespace(node.value, var_type, node):
                ...
                return True
            elif node_type == ast.Subscript and isinstance(node, node_type) and self.query_namespace(node.value, var_type, node):
                ...
                if isinstance(node.value, ast.Name):
                    self.safe.add(node.value)
                return True
            elif node_type == ast.UnaryOp and isinstance(node, node_type) and isinstance(node.op, value) and self.query_namespace(node.operand, var_type, node):
        ...
        if disallow_args:
            for keyword in parent.keywords:
                for banned_key, banned_value in disallow_args.items():
                    if keyword.arg == banned_key and isinstance(keyword.value, ast.Constant) and keyword.value.value == banned_value:
                        return False
        if need_args:
            for needed_key, needed_value in need_args.items():
                has_arg = False
                for keyword in parent.keywords:
                    if keyword.arg == needed_key and isinstance(keyword.value, ast.Constant) and keyword.value.value == needed_value:
                        has_arg = True
                if not has_arg:
                    return False
        return True
